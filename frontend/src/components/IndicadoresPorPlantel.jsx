...
const IndicadoresPorPlantel = () => {
  const [datos, setDatos] = useState([]);
  const [filtros, setFiltros] = useState({ colegio: '', tipo: '', cct: '' });
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    mapaService.getIndicadoresPorPlantel()
        ...
        <table className="tbl-main">
          <thead>
            <tr>
              ...
              <th>Desafiliación Escolar 2024-2025</th>
              <th>Reprobación 2024-2025</th>
            </tr>
          </thead>
          <tbody>
            {datosFiltrados.length === 0 ? (
              ...
              paginados.map(row => (
                <tr key={row.id}>
                  ...
                  <td>{row.nombreDelPlantel}</td>
                  <td className="tbl-num">{row.eficienciaTerminal ?? '—'}</td>
                  <td className="tbl-num">{row.desafiliacionEscolar ?? '—'}</td>
                  <td className="tbl-num">{row.reprobacion ?? '—'}</td>
                </tr>
              ))
            )}
          </tbody>
...
export default IndicadoresPorPlantel;
